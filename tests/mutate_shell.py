...
MUTS = [
 ...
 ("Mirabellenmenue fehlt",
  '    s->menus[0] = (menu){ "menu.mirabelle", MIRABELLE_ITEMS, 1 };',
  '    s->menus[0] = (menu){ "menu.file", FILE_ITEMS, 6 };'),
 ("Ueber-Dialog geht nicht auf",
  '        s->ask     = dialog_open(s->wm, s->cfg.catalog, "dialog.about.body",\n                                 NULL, 0, btns, 1);',
  '        s->ask     = NULL; (void)btns;'),
 ("zweiter Dialog ueber dem ersten",
  '    if (strcmp(action, "app.about") == 0) {\n        if (s->ask) return;',
  '    if (strcmp(action, "app.about") == 0) {\n        if (false) return;'),
 ("Titel bleibt in der Einzelansicht die Sammlung",
  '    if (browser_view_of(a->br) == BROWSE_FORM) {', '    if (false) {'),
 ("Kalender nennt die Sammlung statt des Monats",
  '        snprintf(title, sizeof title, "%04d-%02d", m.year, m.month);',
  '        snprintf(title, sizeof title, "%s", T(s->cfg.catalog, a->label));'),
 ("Monat ohne fuehrende Null",
  '        snprintf(title, sizeof title, "%04d-%02d", m.year, m.month);',
  '        snprintf(title, sizeof title, "%d-%d", m.year, m.month);'),
 ("Skripttitel wird nicht gefragt",
  '        if (s->cfg.scripts && s->cfg.scripts->window_title)\n            what = s->cfg.scripts->window_title(s->cfg.scripts->user, a->script);',
  '        (void)0;'),
 ("leerer Skripttitel zaehlt",
  '        if (what && *what) compose_title(what, type, title, sizeof title);',
  '        if (what) compose_title(what, type, title, sizeof title);'),
 ("Typ faellt aus dem Titel",
  '    if (what && *what && type && *type) snprintf(out, n, "%s (%s)", what, type);',
  '    if (false) snprintf(out, n, "%s (%s)", what, type);'),
 ("fehlender Singular wird angezeigt",
  '    if (i18n_has(s->cfg.catalog, key)) Tn(s->cfg.catalog, label, 1, out, n);',
  '    if (true) Tn(s->cfg.catalog, label, 1, out, n);'),
 ...
 ("Skript sieht seine Taste nicht",
  ...
  "        (void)cur;"),
 ("auch ohne Datensatz verbraucht",
  "    return strncmp(action, \"record.\", 7) == 0 ||",
  "    return false ||"),
 ("Formulartasten ohne Datensatz verbraucht",
  "           strncmp(action, \"form.\", 5) == 0 ||",
  "           false ||"),
 ("Return ohne Liste verbraucht",
