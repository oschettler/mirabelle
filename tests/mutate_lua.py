...
DEPS = ["src/app/schema.c", "src/app/fieldkind.c",
        ...
        "src/ui/theme.c", "src/ui/caret.c", "src/ui/widget.c", "src/ui/widget_list.c",
...
MUTS = [
 ...
 ("pdalua_schema.c", "langer Name wird abgeschnitten",
  '    if (strlen(s) >= cap) {', '    if (false) {'),
 ("pdalua_schema.c", "beliebig viele Felder",
  '        if (s->field_count >= SCHEMA_FIELDS_MAX) {', '        if (false) {'),
 ("pdalua_schema.c", "unbrauchbarer Listeneintrag erlaubt",
  '        if (!s || strlen(s) >= SCHEMA_NAME_MAX) {', '        if (!s) {'),
 ("pdalua.c", "clip ohne Angaben schraenkt nicht auf",
  '        gc_clip(g, rect_make(0, 0, g->dst->w, g->dst->h));\n        return 0;',
  '        return 0;'),
 ("pdalua_widgets.c", "Anzeige zeichnet nicht",
  '    if (g) widget_draw(v->w, g);', '    (void)g;'),
 ("pdalua_widgets.c", "Anzeige bekommt keine Ereignisse",
  '    lua_pushboolean(L, widget_event(v->w, &e));', '    lua_pushboolean(L, 0);'),
 ("pdalua_widgets.c", "Anzeige haelt den Text nicht fest",
  '    lua_pushvalue(L, 2);\n    lua_setiuservalue(L, 1, 1);\n\n    gemview_set_text(v->w, text, len);',
  '    gemview_set_text(v->w, text, len);'),
 ("pdalua_widgets.c", "Anzeige bekommt keinen Fokus",
  '    v->w->focused = true;', '    v->w->focused = false;'),
 ("pdalua_widgets.c", "Balken haengt an keiner Anzeige",
 ...
 ("pdalua_widgets.c", "Anzeige wird nicht festgehalten",
  '    if (v) {\n        lua_pushvalue(L, 1);\n        lua_setiuservalue(L, -2, 1);\n    }', '    (void)0;'),
 ("pdalua_widgets.c", "Verweisnummer ab null",
  '    const char *url = gemview_link_url(v->w, (int)luaL_checkinteger(L, 2), &len);',
  '    const char *url = gemview_link_url(v->w, (int)luaL_checkinteger(L, 2) + 1, &len);'),
 ("pdalua_widgets.c", "Merker wird nicht zurueckgesetzt",
  '    lua_pushboolean(L, gemview_was_opened(check_view(L, 1)->w));',
  '    lua_pushboolean(L, gemview_selected_link(check_view(L, 1)->w) != 0);'),
 ...
 ("pdalua_widgets.c", "Rollposition kommt nicht an",
  '    scroll_by(bar_model(L, 1), (int)luaL_checkinteger(L, 2));', '    (void)L;'),
