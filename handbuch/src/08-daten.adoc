...
**Die Daten überleben das Programm.** Stirbt mirabelle in zwanzig
...
Funktion.** Spartan ist das einfache Netzwerkprotokoll, mit dem mirabelle
später Seiten aus dem Netz abrufen und darstellen kann, und
...
Projekt sich damit erspart. Die zulässige Grammatik steht im Kopf von
`src/store/frontmatter.h` — dort, wo sie niemand übersehen kann, der den Parser
anfasst — und wird gegen eine Sammlung gültiger und ungültiger Beispiele
getestet — derselbe Grundsatz wie beim Gemtext-
...
Jetzt zum eigentlichen Kern: Aufgaben, Kontakte und Notizen sind in
mirabelle *derselbe Code*. Was sie unterscheidet, ist nicht Programmlogik,
sondern eine Datendatei — ein Schema. So sieht das Schema für Aufgaben aus,
gekürzt um die Kommentare:
...
`Aufgaben/`, wie im Verzeichnisbaum oben. `fields` listet jedes
Front-Matter-Feld mit seiner Art (`kind`) auf: Text, Datum, eine Auswahl aus
festen Werten, ein Wahrheitswert. `columns` sind die Spalten der Liste, `form`
die Felder des Formulars in der Reihenfolge, in der man sie eingibt, und `sort`
das Feld, nach dem sortiert wird. `title_field` sagt, welches Feld einen
einzelnen Datensatz benennt — es steht im Fenstertitel, sobald der Datensatz
allein zu sehen ist.

Genau ein Feld darf `gemtext` sein. Es ist kein Feld im Front Matter, sondern
der Körper des Datensatzes — und einen Körper hat ein Datensatz nur einmal.

.Die Datei ist Lua, der Vertrag ist es nicht
[NOTE]
====
Gelesen wird die Datei von Lua, weil ein eigenes Format ein eigener Parser
wäre, mit eigenen Meldungen und eigenen Grenzen — alles davon ein zweites Mal
richtig zu machen. Was dabei herauskommt, ist aber eine C-Struktur (`schema` in
`src/app/schema.h`), und die ist der Vertrag: der Browser sieht der Anwendung
nicht an, woher sie kam. Das ist Entscheidung D-15.

Der Preis dafür steht in `CMakeLists.txt`: ohne Lua gibt es keine Schemata,
ohne Schemata keine Anwendung. Deshalb bricht das Bauen ab, statt still etwas
Unbrauchbares zu erzeugen.
====

Der generische Browser (`src/app/browser.c`) liest ein solches Schema und baut
daraus Liste, Formular, Menüs und Tastenkürzel — für Aufgaben, für Kontakte und
für Notizen entsteht so eine vollständige Anwendung *ohne eine einzige Zeile
eigenen Programmcodes*. Auch die Feldtypen sind keine Sonderfälle im Programm,
sondern Einträge in einer Registratur (`src/app/fieldkind.c`): jeder Typ sagt,
wie er einen Wert anzeigt, wie er ihn zurückliest und welches Bedienelement er
dafür braucht. Ein neuer Feldtyp ist ein Eintrag mehr, keine Verzweigung mehr.
...
. Schreib von Hand eine `.gmi`-Datei für eine Notiz, die du wirklich
  ...
  Liste und einem Verweis mit `=>`. Leg sie unter `~/PDA/Notizen/` ab und
  starte `./build/mirabelle`: erscheint sie in den Notizen? Prüfe danach am Text,
  ob jede Zeile eindeutig einem der sechs Zeilentypen zuzuordnen ist, ohne
  dass du dafür wissen musst, was in einer anderen Zeile steht.
...
. Schreib ein Schema für Rezepte, im selben Stil wie `task.lua` oben: welche
  Felder braucht ein Rezept (Zutaten, Zubereitungszeit, Portionen, Kategorie
  …), welchen `kind` hat jedes davon, was gehört in `columns`, was in `form`?
  Leg es als `data/schema/rezept.lua` ab und starte `./build/mirabelle --apps`.
  Erscheint deine Anwendung? Wenn nicht, sagt die Meldung, warum.
