...
Fragen mit derselben Liste beantwortet — und welche zwei Regeln über
Lebensdauern daran hängen.
...
Die letzte Zeile ist die entscheidende: es wandert der Umriss, nicht das
Fenster. Erst bei `EV_MOUSE_UP` übernimmt das Fenster die neue Lage:
...
=== Zwei Regeln über Lebensdauern

Ein Fenstersystem verwaltet Speicher, den andere benutzen: das Thema kommt von
außen, das Fenster geht an einen Eigentümer. Beide Richtungen haben eine Regel,
und beide Regeln stehen im Code, nicht nur hier.

==== Ein Thema wird kopiert, nie als Zeiger gehalten

`wm_create` bekommt ein Thema übergeben und legt eine Kopie davon an
(`src/ui/wm.c`):
...
Ein Thema ist, wie `theme.h` zeigt, eine Handvoll Ganzzahlen — das Kopieren
kostet nichts der Rede wert. Ein Zeiger dagegen kostet eine Zusage, die der
Aufrufer einhalten muss: *dein Thema muss meine Verwaltung überleben.* Diese
Zusage ist leicht zu geben und leicht zu brechen, und zwar auf die
unangenehmste Art.

[WARNING]
====
Ein Thema, das als lokale Variable in einer Hilfsfunktion angelegt und dessen
Adresse weitergereicht wird, existiert nach der Rückkehr dieser Funktion nicht
mehr. Der Speicher liegt auf dem *Stapelspeicher* (englisch „stack"), dem
Bereich, in dem Funktionen ihre lokalen Variablen halten, und wird beim
nächsten Aufruf wiederverwendet.

Was dann passiert, ist kein Absturz. Über einen solchen Zeiger gelesene Maße
sind einfach andere Zahlen — und eine Zeichenschleife über ein Rechteck mit
absurder Kantenlänge stürzt nicht ab, sie läuft weiter. Das Programm scheint
zu hängen, ohne eine Zeile Ausgabe. Ein Absturz wäre die freundlichere
Variante gewesen.
====

Deshalb gilt die Regel im ganzen Projekt und nicht nur hier: **wer ein Widget
oder eine Verwaltung außerhalb eines Panels hält, hält auch eine Kopie des
Themas.** Sie steht in `handbuch/../STAND.md` unter den Arbeitsregeln, weil
diese Falle mehr als eine Stelle betrifft.

==== Wer ein Fenster besitzt, erfährt von seinem Ende

Die Fensterverwaltung schließt ein Fenster *selbst*, sobald der Nutzer das
Schließfeld trifft: `wm_event` ruft dafür intern `wm_close`. Der Eigentümer —
die Anwendung, die den Zeiger hält — erführe davon nichts und zeichnete beim
nächsten Bild in freigegebenen Speicher.

Die Antwort darauf ist eine Rückmeldung, deklariert in `src/ui/window.h`:
...
Entscheidend ist der erste Satz: „gleich ob über das Schließfeld, über
`wm_close` oder beim Abräumen der ganzen Verwaltung". Es gibt drei Wege, wie
ein Fenster verschwinden kann, und ein Eigentümer, der nur einen davon kennt,
vergisst irgendwann einen der anderen.

Die Antwort darauf ist kein Aufruf an drei Stellen, sondern ein einziger — an
der Stelle, durch die *alle* drei Wege zwangsläufig laufen
(`src/ui/window.c`):
...
`wm_close` ruft `window_destroy` auf, `wm_destroy` ruft es beim Abräumen für
jedes verbliebene Fenster auf, und der Klick auf das Schließfeld führt über
`wm_close` zum selben Punkt. Damit die Rückmeldung *vor* der Freigabe kommt,
steht der Aufruf als Erstes in der Funktion, noch vor `bitmap_free`.

Das ist dieselbe Bauform wie das Clipping aus Kapitel 5: eine Sache, die
überall gelten muss, wird an genau einer Stelle geprüft beziehungsweise
gemeldet — nicht an jeder Stelle, an der sie gebraucht wird.
...
. Schreibe einen kleinen Test nach dem Vorbild von `tests/unit/test_wm.c`,
  öffne ein Fenster und richte `window_set_on_close` ein. Schließe es einmal
  ...
  Danach lass die Fensterverwaltung zerstören, ohne vorher zu schließen: auch
